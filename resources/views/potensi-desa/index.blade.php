                ...
                <div class="col-lg-12 col-md-12 col-12">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h3 class="mb-0 text-white">
                            <i class="bi bi-tree-fill me-2"></i> Data Potensi Desa
                        </h3>
                        <a href="{{ route('potensi-desa.create') }}" class="btn btn-light shadow-sm">
                            ...
                            <tbody>
                                @forelse($potensis as $index => $item)
                                    <tr class="align-middle">
                                        ...
                                        <td class="fw-semibold">{{ $item->judul_potensi }}</td>
                                        <td>
                                            @if ($item->gambar)
                                                <img src="{{ asset('storage/' . $item->gambar) }}"
                                                    alt="Gambar {{ $item->judul_potensi }}" class="rounded shadow-sm"
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('potensi-desa.edit', $item->id) }}"
                                                class="btn btn-sm btn-light-warning border-0 me-1 shadow-sm">
                                                <i class="bi bi-pencil text-warning"></i>
                                            </a>

                                            <form id="delete-form-{{ $item->id }}"
                                                action="{{ route('potensi-desa.destroy', $item->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-4">
                                            <i class="bi bi-info-circle me-2"></i> Tidak ada data potensi desa.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
